...
Para fazer uma demonstração do observer foi feito o seguinte código.
...
```
O objetivo desse código é demonstrar a classe `NotificationPublisher` em ação.
Para isso foi inscrito na classe três usuário com diferentes tipos de serviços de notificação, e então a função `notify_all_subscribers` foi chamada produzindo o seguinte resultado.

![Saida observer](./assets/saida-observer.png)

O código acima é um mero exemplo. A classe em questão será utilizada no projeto junto com a classe `Festa`. Dessa forma, todos os convidados inscritos na festão receberão notificações com atualizações das informações da festa.
